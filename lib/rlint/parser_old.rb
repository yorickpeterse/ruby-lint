  ...
  class Parser < Ripper::SexpBuilderPP
    ...
    def on_class(klass, parent, body)
      ...
      if parent.is_a?(Rlint::Token::Token)
        ...
        parent_class = parent.value
      elsif parent.is_a?(Array)
        parent_class = parent.map { |token| token.name }.join('::')
        parent_class = parent.map { |token| token.value }.join('::')
      end
    ...
    #
    # @since  2012-07-29
    # @param  [Array] var Array containing details about the variable.
    ...
    def on_assign(var, value)
      ...
      # VariableToken class.
      if var.class == Rlint::Token::AttributeToken
        var.value = value

        return var
      end

      # TODO: this really need to be done more properly.
      key = nil
      ...
      if var.is_a?(Array)
        ...
        name   = var.name
        type   = var.respond_to?(:type) ? var.type : :local_variable
        line   = var.line
        column = var.column
        code   = var.code
      end
    ...
    #
    # @since  2012-07-29
    # @param  [Rlint::Token::Token] var Token class containing details
